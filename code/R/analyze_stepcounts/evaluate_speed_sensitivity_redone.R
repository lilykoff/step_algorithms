...
manual_cols = c("steps_adept" = coladept, "steps_sdtnew" = colsdt,
...
marea = read_csv(here::here("results/all_algorithms/marea_step_estimates_1sec_redone.csv.gz")) %>%
  select(-contains("raw")) %>%
  select(-c(steps_sdt_30, steps_sdt))
...
names(labs) = c("steps_acti", "steps_adept", "steps_oak",  "steps_scrf", "steps_scssl","steps_sdtnew", "steps_vsores", "steps_vsrres")
...
svg(here::here("manuscript/figures","speed_redone.svg"))
plt
dev.off()
plt
