...
manual_cols = c("adept" = coladept, "sdtnew" = colsdt,
                "oak" = coloak, "acti" = colacti,
                "vsores" = colvso, "vsrres" = colvsr,
                "scssl" = colscs,  "scrf" = colscr)
  clemson = readr::read_csv(here::here("results/all_algorithms/clemson_step_estimates_1sec_redone.csv.gz"))
  # just use 100 hz data from oxwalk for this part
  oxwalk = readr::read_csv(here::here("results/all_algorithms/oxwalk_step_estimates_1sec_redone.csv.gz")) %>%
    filter(sample_rate == 100)
  # remove running from MAREA
  marea = readr::read_csv(here::here("results/all_algorithms/marea_step_estimates_1sec_redone.csv.gz")) %>%
    filter(grepl("run", cat_activity)==FALSE)
  cor(clemson$steps_sdt, clemson$steps_sdtnew)

 cor(marea$steps_sdt, marea$steps_sdtnew)
cor(oxwalk$steps_sdt, oxwalk$steps_sdtnew)

clemson %>%
  group_by(id_subject) %>%
  summarize(across(starts_with("step"), sum)) %>%
  summarize(c = cor(steps_sdtnew, steps_sdt))
...
oxwalk %>%
  ...
  step_df =
    readRDS(here::here("results", "all_algorithms", "step_stats_bysubject_redone.rds")) %>%
  ...
  tab_individual %>%
    ...
    filter(algorithm == "steps_sdtnew_30")
  tab_individual %>%
    left_join(tab_overall, by = c("metric", "algorithm")) %>%
    select(metric, algorithm, clemson_overall, marea, oxwalk100, overall) %>%
  ...
  total_steps = readRDS(here::here("results", "all_algorithms", "total_steps_bysubject_redone.rds")) %>%
    filter(algorithm %notin% c("steps_vsoraw_30", "steps_vsrraw_30")) %>%
    filter(!grepl("_sdt_", algorithm)) %>%
    filter(cat_activity != "oxwalk25" &
             (grepl("30", algorithm) | grepl("truth", algorithm)))
  labs = c("ActiLife", "ADEPT", "Oak", "stepcount RF", "stepcount SSL", "SDT", "Verisense original",
           "Verisense revised")

  names(labs) = c("acti", "adept", "oak",  "scrf", "scssl","sdtnew", "vsores", "vsrres")
  ...
  svg(here::here("manuscript/figures", "truth_v_predicted_redone.svg"))
