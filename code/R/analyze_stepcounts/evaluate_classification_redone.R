...
manual_cols = c("steps_adept_30" = coladept, "steps_sdtnew_30" = colsdt,
...
accuracy_df = readRDS(here::here("results", "all_algorithms", "accuracy_stats_bysubject_redone.rds")) %>%
  ungroup() %>%
  filter(!(algorithm %in% c("steps_sdt", "steps_sdt_30")))
  ...
  plot = overall %>%
    ...
    scale_x_discrete(labels = c("ADEPT",  "Oak", "SDT", "ActiLife", "Verisense orig.", "Verisense rev.", "stepcount RF", "stepcount SSL"))
  ...
  svg(here::here("manuscript/figures", "boxplots_both_redone.svg"))
  ...
  # significance testing
  pairs = expand.grid.unique(x = c("acti", "adept", "oak", "scrf", "scssl", "sdtnew", "vsores", "vsrres"),
                             y =  c("acti", "adept", "oak", "scrf", "scssl", "sdtnew", "vsores", "vsrres")) %>%
    as_tibble() %>%
