...
all_clem =
      ...
      .f = function(id){
        ...
        map(.x = activities,
            .f = function(activity){
              ...
              sdt = readr::read_csv(step_files_raw[grepl("sdt", step_files_raw) & !grepl("new", step_files_raw)])
              sdtnew = readr::read_csv(step_files_raw[grepl("sdtnew", step_files_raw)])
              ...
              sdt_r = readr::read_csv(step_files_resamp[grepl("sdt", step_files_resamp) & !grepl("new", step_files_resamp)])
              sdtnew_r = readr::read_csv(step_files_resamp[grepl("sdtnew", step_files_resamp)])
  ...
  bind_rows()
saveRDS(all_clem, here::here("data/processed", "clemson_nested_all_redone.rds"))
rm(all_clem)
...
saveRDS(all_ox, here::here("data/processed", "oxwalk_nested_all_redone.rds"))
rm(all_ox)
...
if(length(all_marea_files) > 0){
  all_marea =
    ...
    bind_rows()
  saveRDS(all_marea, here::here("data/processed", "marea_nested_all_redone.rds"))
  rm(all_marea)
}
...
clem = readRDS(here::here("data/processed/clemson_nested_all_redone.rds")) %>%
...
readr::write_csv(clem, here::here("results/all_algorithms/clemson_step_estimates_1sec_redone.csv.gz"))


ox = readRDS(here::here("data/processed/oxwalk_nested_all_redone.rds")) %>%
  select(-c(raw_data, resampled_data))
...
readr::write_csv(ox, here::here("results/all_algorithms/oxwalk_step_estimates_1sec_redone.csv.gz"))

if(file.exists(here::here("data/processed/marea_nested_all_redone.rds"))){
  marea = readRDS(here::here("data/processed/marea_nested_all_redone.rds")) %>%
    select(-c(raw_data, resampled_data))
  ...
  readr::write_csv(marea, here::here("results/all_algorithms/marea_step_estimates_1sec_redone.csv.gz"))
}
